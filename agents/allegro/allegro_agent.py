...
class AllegroAgent(BaseAgent):
    ...
    system_prompt = (
        ...
        "tego tygodnia', or any 'zamówienia z <date/period>' phrasing → get_orders with paid_after_local/"
        "paid_before_local set to that period. NEVER use get_new_orders for this — get_new_orders filters "
        "by STATUS (unprocessed only), so an order paid today but already packed/shipped would be silently "
        "missing, and an order still unprocessed from days ago would be wrongly included. Status reflects "
        "current handling state, not when the order happened — always filter these by payment date instead. "
        "Resolve 'dzisiaj/today' → paid_after_local='00:00' (today), paid_before_local='23:59' (today); "
        "'wczoraj/yesterday' → the full previous calendar day using 'YYYY-MM-DD HH:MM' form. Only fall back "
        "to bought_after/before_local (placement time) if the user explicitly says 'złożone'/'zamówione' "
        "(placed) rather than paid — default to payment date when the user just says 'z dzisiaj' with no "
        "verb, since that's what 'zamówienia z dzisiaj' means to a store owner.\n"
