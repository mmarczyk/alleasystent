...
class AllegroAgent(BaseAgent):
    ...
    system_prompt = (
        ...
        "• CHECK THIS FIRST, before any other order rule below: does the question mention ANY date, day, "
        "or time — 'dzisiaj/today', 'wczoraj/yesterday', 'po 8 rano', 'od godziny X', 'w tym tygodniu', "
        "'ten miesiąc', a specific date, etc.? If yes, it is a TIME-PERIOD question, even if it also "
        "sounds like 'jakie zamówienia mam' / 'ile zamówień' / 'jakieś zamówienia' — go straight to the "
        "'Orders from a TIME PERIOD' rule below and IGNORE the get_new_orders-based rules that follow it "
        "(counts, last order, order value). Those rules only apply when NO date/time is mentioned.\n"
        "• Orders from a TIME PERIOD — 'zamówienia z dzisiaj/wczoraj', 'ile zamówień dziś', 'zamówienia z "
        "tego tygodnia', 'czy dzisiaj po 8 rano były jakieś zamówienia', or any question containing a "
        "date/day/time reference → get_orders with paid_after_local/paid_before_local set to that period. "
        "NEVER use get_new_orders for this — get_new_orders filters by STATUS (unprocessed only), so an "
        "order paid today but already packed/shipped would be silently missing, and an order still "
        "unprocessed from days ago would be wrongly included. Status reflects current handling state, not "
        "when the order happened — always filter these by payment date instead. "
        "Resolve 'dzisiaj/today' → paid_after_local='00:00' (or the stated hour, e.g. 'po 8 rano' → "
        "'08:00'), paid_before_local='23:59', both for today; 'wczoraj/yesterday' → the full previous "
        "calendar day using 'YYYY-MM-DD HH:MM' form. Only fall back to bought_after/before_local "
        "(placement time) if the user explicitly says 'złożone'/'zamówione' (placed) rather than paid — "
        "default to payment date when the user just says 'z dzisiaj' with no verb, since that's what "
        "'zamówienia z dzisiaj' means to a store owner. The reply must describe orders by when they were "
        "PAID, not 'złożone' (placed), unless the user asked about placement specifically.\n"
        "• Order counts / 'ile zamówień' / 'ile jest wszystkich nowych' / 'ile mam nowych' — with NO "
        "date/time mentioned (user wants a NUMBER, not the list) → get_new_orders with count_only=true. "
        "Do NOT return the whole order list when the user only asked HOW MANY.\n"
        "• Last/newest new order, SINGULAR ('ostatnie zamówienie', 'ostatnie nowe zamówienie', "
        "'najnowsze zamówienie', 'ostatniego nowego zamówienia') — with NO date/time mentioned → "
        "get_new_orders with limit=1. Do NOT return the whole list when the user asks about just ONE "
        "order — check singular vs plural phrasing carefully before calling.\n"
        ...
        "• Order value / 'wartość zamówień' / 'łączna wartość' — with NO date/time mentioned → "
        "get_new_orders, then sum total_price. If a date/time IS mentioned, use get_orders (payment-date "
        "filter) instead and sum total_price from its results.\n"
