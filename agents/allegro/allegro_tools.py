...
ALLEGRO_TOOLS: list[dict] = [
    {
        ...
        "function": {
            ...
            "description": (
                ...
                "but ONLY when no date/time/period is mentioned. "
                "Automatically filters for READY_FOR_PROCESSING + fulfillment=NEW orders — this is a "
                "STATUS filter, not a date filter, so results can include orders from any day and can "
                "exclude orders from today that are already processed. "
                "DO NOT use this tool if the question mentions a date, day, or time at all — e.g. "
                "'dzisiaj', 'wczoraj', 'dziś po 8 rano', 'od wczoraj', 'w tym tygodniu', 'czy dzisiaj "
                "były jakieś zamówienia' — even though those sound like 'jakie zamówienia mam'/'jakieś "
                "zamówienia' phrasing. ANY mention of a date/time/period means use get_orders with "
                "paid_after_local/paid_before_local instead — status has nothing to do with when an "
                "order happened. "
