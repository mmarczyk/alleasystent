...
CASES: list[Case] = [
    ...
    Case("get_orders", "get_orders", {"limit": 10},
         ...
         ("**Zamówienie**", "Status:", "Wysyłka do:", "Link:"),
         note="Pełna lista — ten sam blok zamówienia co pozostałe listowania."),
    ...
    Case("get_orders__dispatch_deadline", "get_orders",
         {"dispatch_before_local": f"{_TODAY_ISO} 23:59", "limit": 50},
         "Które zamówienia muszę wysłać dzisiaj?",
         ("**Zamówienie**", "Wysyłka do:"),
         note="Filtr terminu nadania (dispatch_before_local) — po stronie klienta."),
    Case("get_orders__count", "get_orders", {"count_only": True},
    ...
    Case("calculate_order_profit", "calculate_order_profit",
         ...
         ("- Przychód (kwota od kupującego): 429,98 PLN",
          "  - Razem koszt towaru: 16,20 PLN",
          "- Opłaty Allegro: -52,20 PLN",
          "**Zysk: 361,58 PLN**"),
         note="Zysk = przychód − opłaty Allegro − koszt zakupu podany przez sprzedawcę "
              "(jedyna liczba, której Allegro nie zna)."),
    Case("calculate_order_profit__per_product", "calculate_order_profit",
         {"order_id": ds.ORD_1, "item_costs": [
             {"offer_id": "14587236901", "unit_cost": 250},
             {"offer_name": "filtr wody", "unit_cost": 8.1},
         ]},
         "Ekspres kosztował mnie 250 zł, a filtr 8,10 zł — ile zarobiłem na tym zamówieniu?",
         ("  - Razem koszt towaru: 258,10 PLN", "**Zysk: 119,68 PLN**"),
         note="Różne koszty na pozycję: dopasowanie po ID oferty i po nazwie."),
    Case("calculate_order_profit__no_cost", "calculate_order_profit", {"order_id": ds.ORD_1},
         "Ile zarobiłem na tym zamówieniu?",
         ("koszt 1 szt.",), no_api=True,
         note="Bez podanego kosztu zakupu narzędzie pyta, zamiast liczyć z kosztem zerowym "
              "— czyli podać cały przychód jako zysk."),
    Case("get_orders_delivery", "get_orders_delivery", {},
         "Które zamówienia czekają na wysyłkę?",
         ("Podsumowanie kurierów", "**Zamówienie**", "Numer śledzenia", "Punkt odbioru"),
         note="Gotowe do wysyłki — ten sam blok zamówienia + kurierzy i linki do śledzenia."),
    ...
    Case("get_active_offers", "get_active_offers", {},
         ...
         ("# Aktywne oferty", "| Nazwa | Stan (szt.) | Cena | ID ofert |",
          "zakończona — wyprzedana", "Masz obecnie"),
         note="Gotowa tabela + zdanie podsumowania na końcu (to ono trafia do dymka czatu)."),
    Case("get_active_offers__name", "get_active_offers", {"name": "ekspres"},
         "Pokaż oferty z ekspresami",
         ("Ekspres do kawy", "filtr nazwy: „ekspres”"),
         note="Filtr po nazwie — zapytanie do API z parametrem name."),
    Case("get_offers_summary", "get_offers_summary", {},
         "Podsumuj moje oferty",
         ("## Podsumowanie ofert", "## Stany magazynowe", "## Ceny", "```chart"),
         note="Dashboard — koszyki stanów i cen razem z wykresami."),
    Case("query_offers_by_stock", "query_offers_by_stock", {"max_stock": 5},
         "Które produkty mają mniej niż 5 sztuk?",
         ("# Stany magazynowe (≤ 5 szt.)", "| Nazwa | Stan (szt.) | Cena | ID ofert |", "Znaleziono"),
    ...
    Case("query_offers_by_price", "query_offers_by_price", {"min_price": 100, "max_price": 600},
         ...
         ("# Oferty wg ceny", "| Nazwa | Cena | Stan (szt.) | ID oferty |", "Znaleziono"),
    ...
    Case("get_products_to_reorder", "get_products_to_reorder", {"max_stock": 5},
         ...
         ("# Zamówienie uzupełniające", "Do uzupełnienia:", "zakończona — wyprzedana"),
         note="Dokument z tabelą braków, razem z produktami wyprzedanymi do zera."),
    Case("get_offer_details", "get_offer_details", {"offer_id": "14587236901"},
         "Pokaż szczegóły oferty 14587236901",
         ("- ID oferty:", "- Cena:", "- Stan magazynowy:", "- Link:"),
         note="Szczegóły oferty: cena, stan, status publikacji, link."),
    Case("update_offer_price", "update_offer_price",
         {"offer_id": "14587236901", "price": 379.99},
         "Zmień cenę oferty 14587236901 na 379,99 zł",
         ("✅ Cena oferty", "379,99 PLN"),
    ...
    Case("update_offer_price__invalid", "update_offer_price",
         ...
         ("cena musi być większa od zera",), no_api=True,
    ...
    Case("get_message_threads", "get_message_threads", {},
         ...
         ("nieprzeczytana", "ostatnia wiadomość:", "wątek `"),
         note="Lista wątków: kupujący, status, data, ID wątku (potrzebne do odpowiedzi)."),
    Case("get_message_threads__count", "get_message_threads", {"count_only": True},
         "Czy mam nowe wiadomości?",
         ("Masz **2** nowe wiadomości", "Pokazać szczegóły?"),
    ...
    Case("get_thread_messages__by_buyer", "get_thread_messages",
         ...
         ("kupujący **marek_zielinski**", "odkurzacza"),
    ...
    Case("issue_invoice_for_order", "issue_invoice_for_order", {"order_id": ds.ORD_1},
         ...
         ("✅ Faktura", "infakt.pl/share", "firma", "NIE dołączyłem", "potwierdzenie"),
         note="Wystawienie faktury kończy się na inFakt — do Allegro i do KSeF trafia "
              "dopiero po tym, jak użytkownik obejrzy fakturę i potwierdzi, że jest OK."),
